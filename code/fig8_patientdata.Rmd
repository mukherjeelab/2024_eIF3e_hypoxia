...
patient_ye_and_3e <- left_join(sig_zscores_3e, sig_zscores_hyp_signature, by="patient") %>% 
  rename("hypoxia_z_score"="z_score") %>% 
   rename("eIF3e_z_score"="total_z_score")

ggplot(patient_ye_and_3e, aes(x=hypoxia_z_score, y= eIF3e_z_score))+
  geom_point()+
...
patient_ye_and_3e_hyp_clin$quadrant <- factor(patient_ye_and_3e_hyp_clin$quadrant, levels=c("low hypoxia and low eIF3e (hypoxia)", "high hypoxia and low eIF3e (hypoxia)", "low hypoxia sig and high eIF3e (hypoxia)", "high hypoxia and high eIF3e (hypoxia)"))
...
In my 3e hyp signature, there eare:
78 up genes
77 down genes
...
```
Proportion of significant p-values: 441/1000 -- this is insignificant. -- what went wrong here?
...
# merged_tcga_zscores_matrix <- merged_zscores_tcga %>% 
#   column_to_rownames("Hugo_Symbol") %>% 
#   as.matrix()
...
ggsave("tcga_3e.pdf", path=here("plots/fig8"),width = 11.5, height=8)
...
ggsave("tcga_3d.pdf", path=here("plots/fig8"),width = 11.5, height=8)
...
old pos genes = 1747 genes
old neg genes = 1638 genes
...
## Looking at copy number alterations in data - probably delete for manuscript
