...
# a209_norm_total <- big_comparisons$
#   
# a209_hyp_total <- big_comparisons$
...
- this means factor levels are critical - need to be checked, maybe printed, before each analysis

Normoxia. Si3e compared to sicontrol

Flipping so eIF3e is positive
```{r}
...
# export to excel ---------------------------------------------------------------
   ...
   writexl::write_xlsx(x = res_og_df_3e_norm, path = here("counts/rna_signature_3e_normoxia.xlsx"), col_names = T)
...
# export to excel ---------------------------------------------------------------
   ...
   writexl::write_xlsx(x = res_og_df_hypoxia_3e, path = here("counts/rna_signature_3e_hypoxia.xlsx"), col_names = T)
...
# export to excel ---------------------------------------------------------------
   ...
   writexl::write_xlsx(x = res_og_df_normoxia_3d, path = here("counts/rna_signature_3d_normoxia.xlsx"), col_names = T)
...
## Redoing a209 analysis the same way -- high values associated with a209 treatment -- should sortve be opposite of 3e
# a209 normoxia
```{r}
...
# export to excel ---------------------------------------------------------------
   ...
   writexl::write_xlsx(x = res_og_normoxia_a209, path = here("counts/rna_signature_a209_normoxia.xlsx"), col_names = T)
...
# export to excel ---------------------------------------------------------------
   ...
   writexl::write_xlsx(x = res_og_hypoxia_a209, path = here("counts/rna_signature_a209_hypoxia.xlsx"), col_names = T)
...
# Export TE signatures! These are 3e KD signatures or a209 sigantures, so eIF3e low signatures
```{r}
...
write_xlsx(list(eIF3e_normoxia = norm_3e_no_cutoff,
           ...
           here("counts/TE_signatures_no_LFC_cutoff.xlsx"))
...
write_xlsx(list(eIF3e_normoxia = norm_3e_0.5cutoff,
           ...
           here("counts/TE_signatures_0.5_LFC_cutoff.xlsx"))
...
write_xlsx(list(eIF3e_normoxia = norm_3e_1cutoff,
           ...
           here("counts/TE_signatures_1_LFC_cutoff.xlsx"))
```
