...
# 
# ggstripchart(data = chart, x = "sirna", y = "TE", color = "sirna", facet.by = "symbol", shape = "sirna", scales="free_y", palette = c("black",si3d_col,si3e_col), position = position_jitterdodge(1.2)) + 
#   ylab(expression("log"[2]*" TE (ribo/rna)"))
# 
...
chart_genes <- plot_chart %>% 
  select(gene_id, category) %>% unique()
...
ggstripchart(data = common_only, x = "sirna", y = "TE", color = "sirna", facet.by = "symbol", shape = "sirna", scales="free_y", palette = c("black",si3d_col,si3e_col), position = position_jitterdodge(1.2)) + 
  ...
  ggpar(plot, title= "eIF3e and eIF3d regulated genes")
 

ggsave(width = 4, height = 3.25, filename = here("plots/commongenes.pdf"))
...
ggsave(width = 4, height = 1.7, filename = here("plots/dgenes.pdf"))
...
ggstripchart(data = e_spec_only, x = "sirna", y = "TE", color = "sirna", facet.by = "symbol", shape = "sirna", scales="free_y", palette = c("black",si3d_col,si3e_col), position = position_jitterdodge(1.2)) + 
  ...
  ggpar(plot, title= "eIF3e-only regulated genes")

ggsave(width = 4, height = 1.7, filename = here("plots/egenes.pdf"))
...
ggsave(width = 3, height = 2.5, filename = here("plots/eanddgenes.pdf"))
...
ggsave(width = 2, height = 2, filename = here("plots/vennup.pdf"))


ggvenn(norm_te_down, auto_scale = T, fill_color = c(si3d_col,si3e_col), text_size = 3)

ggsave(width = 2, height = 2, filename = here("plots/venndown.pdf"))
...
norm_sig_te <- norm_sig_te %>% 
  mutate(color = ifelse((symbol %in% e_genes=="TRUE"), "si3e_col", 
                        ifelse((symbol %in% d_genes=="TRUE"), "si3d_col",
                        ifelse((symbol %in% common=="TRUE"), "green", "z_black"))))
...
ggscatter(data = norm_sig_te,
          ...
          alpha = 0.5,
          xlim=c(-2,2), ylim = c(-2,2),
          color="color", palette=c("green", si3d_col, si3e_col, "grey"), show.legend = FALSE) +
...
ggsave(width = 4, height = 4, filename = here("plots/normoxia_quadrants.pdf"))



# label the genes that we are pointing out in the prior fig


# make it density instead

# plot <- ggplot(norm_sig_te, aes(x = te_3d, y = te_3e)) +
#  geom_point() +
#  xlim(-1.5, 1.5) +
#  ylim(-1.5, 1.5)
# 
# plot + geom_density_2d_filled(alpha = 0.5)
# 
# 
# plot + geom_density_2d_filled(contour_var = "ndensity")+
#   labs(x = "eIF3d delta TE", y = "eIF3e delta TE")
# 
# 
# 
...
ggscatter(data = norm_te_h,
  ...
  ylim(-2.5,2.5) +
  xlim(-2.5,2.5) + theme_prism(base_size = 14)

ggsave(width = 5, height = 4, filename = here("plots/normoxia_halmarks.pdf"))
...
write_csv(norm_h_table, here("counts/norm_h_table.csv"))
...
ggsave(width = 4, height = 4, filename = here("plots/normoxia_kegg.pdf"))
...
write_csv(norm_kegg_table, here("counts/norm_kegg_table.csv"))
...
ggsave(width = 4, height = 4, filename = here("plots/normoxia_reactome.pdf"))
...
write_csv(norm_react_table, here("counts/norm_react_table.csv"))
