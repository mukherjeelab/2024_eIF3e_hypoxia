...
te_counts <- as.matrix(raw_counts_edit[,grep(pattern = "TE",colnames(raw_counts_edit))])

rna_ribo_counts <- as.matrix(raw_counts_edit[,grep(pattern ="TE", colnames(raw_counts_edit), invert=TRUE)])
...
te_norm <- left_join(te_norm, unique(geneInfo[,c(1,4)]), by="gene_id")
...
Controls for each TE analysis:
1. Extract TE values for eIF3d and eIF3e
2. Visualize normalized RNA and RIBO counts for eFI3d and eIF3e
3. Hand-calculate expected TE and cross reference
