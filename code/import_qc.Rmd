...
@Neel, here, I am not filtering for protein coding only as I do in calculate_te.Rmd.  Is that ok?
...
Heatmap of ribo and RNA in normoxia @kate remove separate ribo and rna heatmaps
```{r}
#KDs
...
PCAs for figure 1 -- @Kate - delete once we decide this is done for usre
```{r}
#kds only pca
...
ggsave(pca, filename=here("plots/qc/pca_with_outlier_normoxia_kds_only.pdf"), width= 4, height=3)
...
ggsave(pca_qc_input_hypoxia_kds_plot, filename=here("plots/qc/pca_hypoxia_kds.pdf"), width= 4, height=3)
```
...
ggsave(pca_qc_input_hypoxia_kds_plot_no_out, filename=here("plots/qc/pca_hypoxia_kds_no_out.pdf"), width= 4, height=3)
```

Heatmap

with outlier- @Kate-- make main heatmap for fig 3?
...
heatmap without outlier - @Kate delete
```{r}
mycor_hyp_kds <- cor(qc_input_hypoxia_kds_no_outlier, method = "pearson")
heatmap_hypoxia <- pheatmap::pheatmap(mat = mycor_hyp_kds)
heatmap_hypoxia
ggsave(heatmap_hypoxia, filename=here("plots/qc/heatmap_hyp_kds_no_outlier.pdf"), width=11.5, height=11)
