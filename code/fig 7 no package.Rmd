...
all_up_3d_norm <- all_sheets$normoxia_si3d %>% 
  filter(log2FoldChange < 0)

all_down_3d_norm <- all_sheets$normoxia_si3d %>% 
  filter(log2FoldChange > 0)

all_up_3e_norm <- all_sheets$normoxia_si3e %>% 
  filter(log2FoldChange < 0)

all_down_3e_norm <- all_sheets$normoxia_si3e %>% 
  filter(log2FoldChange > 0)

#hyp


all_up_3d_hyp <- all_sheets$hypoxia_si3d %>% 
  filter(log2FoldChange < 0)

all_down_3d_hyp <- all_sheets$hypoxia_si3d %>% 
  filter(log2FoldChange > 0)

all_up_3e_hyp <- all_sheets$hypoxia_si3e %>% 
  filter(log2FoldChange < 0)

all_down_3e_hyp <- all_sheets$hypoxia_si3e %>% 
  filter(log2FoldChange > 0)
...
ggsave("3e_surv_norm_plot.pdf", path=here("plots"),width = 11.5, height=8)
```


```{r}
# colnames(merged_zscores_all) <- colnames(merged_zscores_all) <- gsub("\\.", "-", colnames(merged_zscores_all))
```
...
```{r}
all_up_a209_norm <- sig_a209_norm %>% 
  filter(log2FoldChange > 0)

all_down_a209_norm <- sig_a209_norm %>% 
  filter(log2FoldChange < 0)


all_up_a209_hyp <- sig_a209_hyp %>% 
  filter(log2FoldChange > 0)

all_down_a209_hyp <- sig_a209_hyp %>% 
  filter(log2FoldChange < 0)
```
...
cor.test(patient_ye_and_a209_hyp$ye_hyp_sig_up, patient_ye_and_a209_hyp$total_z_score)
```
