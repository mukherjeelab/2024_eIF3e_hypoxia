...
sig_a209_norm <- read_excel(here("counts/normoxia_a209_rnacounts.xlsx")) %>% 
  filter(padj<0.05)


#create up and down lists

up_a209_norm <- sig_a209_norm %>% 
  filter(log2FoldChange > 1)

tally(up_a209_norm)

down_a209_norm <- sig_a209_norm %>% 
  filter(log2FoldChange < -1)
...
norm_a209_genes <- sig_a209_norm$symbol

verified_genes_norm_a209 <- norm_a209_genes[norm_a209_genes %in% rownames(merged_zscores_matrix)]

filtered_expr_norm_a209 <- merged_zscores_matrix[verified_genes_norm_a209, , drop = FALSE]
...
surv_plot_a209_norm <- ggsurvplot(fit_all_a209_norm, data = all_data_a209_norm, size = 1, palette = c("#E7B800", "pink", "purple", "#2E9FDF"),legend.labs = c("depleted for a209 signature: low (Q1)","depleted: medium-low (Q2)", "enriched: medium-high (Q3)", "enriched for 209 signature: high (Q4)"),  break.time.by = 1, ylab="Overall Survival", xlab = "Time in years", xlim = c(0,10),conf.int = TRUE, pval = FALSE, risk.table = TRUE,  risk.table.col = "strata", risk.table.height = 0.25, ggtheme = theme_bw())
...
sig_a209_hyp <- read_excel(here("counts/hypoxia_a209_rnacounts.xlsx")) %>% 
  filter(padj<0.05)


#create up and down lists

up_a209_hyp <- sig_a209_hyp %>% 
  filter(log2FoldChange > 1)

tally(up_a209_hyp)

down_a209_hyp <- sig_a209_hyp %>% 
  filter(log2FoldChange < -1)
...
hyp_a209_genes <- sig_a209_hyp$symbol

verified_genes_hyp_a209 <- hyp_a209_genes[hyp_a209_genes %in% rownames(merged_zscores_matrix)]

filtered_expr_hyp_a209 <- merged_zscores_matrix[verified_genes_hyp_a209, , drop = FALSE]
...
surv_plot_a209_hyp <- ggsurvplot(fit_all_a209_hyp, data = all_data_a209_hyp, size = 1, palette = c("#E7B800", "pink", "purple", "#2E9FDF"),legend.labs = c("depleted for a209 signature: low (Q1)","depleted: medium-low (Q2)", "enriched: medium-high (Q3)", "enriched for 209 signature: high (Q4)"),  break.time.by = 1, ylab="Overall Survival", xlab = "Time in years", xlim = c(0,10),conf.int = TRUE, pval = FALSE, risk.table = TRUE,  risk.table.col = "strata", risk.table.height = 0.25, ggtheme = theme_bw())
...
from Zed's paper
https://www.nature.com/articles/s41467-021-21687-2
...
coxfit <- coxph(Surv(OS_YEARS, OS_STATUS) ~ quantile, all_data_hif)
summary_coxfit_hif<- summary(coxfit)


hr <- round(summary_coxfit_hif$coefficients[1, "exp(coef)"], 2)
pval <- format.pval(summary_coxfit_hif$coefficients[1, "Pr(>|z|)"], digits = 2, eps = 0.001)

surv_plot_hif <- ggsurvplot(fit_all_hif, data = all_data_hif, size = 1, palette = c("#E7B800", "pink", "purple", "#2E9FDF"),legend.labs = c("depleted for HIF signature: low (Q1)","depleted: medium-low (Q2)", "enriched: medium-high (Q3)", "enriched for HIF signature: high (Q4)"),  break.time.by = 1, ylab="Overall Survival", xlab = "Time in years", xlim = c(0,10),conf.int = TRUE, pval = FALSE, risk.table = TRUE,  risk.table.col = "strata", risk.table.height = 0.25, ggtheme = theme_bw())



surv_plot_hif$plot <- surv_plot_hif$plot + 
  annotate(
    "text", 
    x = 3, y = 0.2, 
    label = paste("HR =", hr, "\np =", pval), 
...
```{r}
patient_hif_and_3e <- left_join(patient_scores, hif_patients, by="patient")
...
```{r}
ggplot(patient_hif_and_3e, aes(x=total_hif_z_score, y= total_z_score))+
  geom_point()+
...
cor.test(patient_hif_and_3e$total_hif_z_score, patient_hif_and_3e$total_z_score)
```
...
patient_ye_and_3e_hyp <- patient_ye_and_3e_hyp %>% 
  mutate(quadrant = ifelse(ye_hyp_sig_up < 0 & total_z_score < 0, "depleted hypoxia sig and depleted eIF3e (hypoxia) KD sig",
                           ifelse(ye_hyp_sig_up > 0 & total_z_score < 0, "enriched hypoxia sig and depleted eIF3e (hypoxia) KD sig",
                                  ifelse(ye_hyp_sig_up > 0 & total_z_score > 0, "enriched hypoxia sig and enriched eIF3e (hypoxia) KD sig",
                                         ifelse(ye_hyp_sig_up < 0 & total_z_score > 0, "depleted hypoxia sig and enriched eIF3e (hypoxia) KD sig", "other")))))
