...
#stopifnot(
#identical(hyp_3d$gene_id,hyp_3e$gene_id))

hyp_sig_te <- data.frame(
  gene_id = hyp_te_3d$gene_id,
...
hyp_hallmark <- hyp_te_h %>% 
  mutate(category= ifelse(eif3d_pvalue< 0.05 & eif3e_pvalue <0.05, "both",
                          ifelse(eif3d_pvalue< 0.05 & eif3e_pvalue >0.05, "d_only",
                          ifelse(eif3d_pvalue> 0.05 & eif3e_pvalue <0.05, "e_only", "NA")))) %>% 
  relocate(category, .after = ID)



DT::datatable(hyp_hallmark) %>%
  formatRound(which(sapply(hyp_te_h,is.numeric)), digits = 3)

write_csv(hyp_hallmark, here("counts/hyphallmarkt.csv"))
...
DT::datatable(hyp_kegg) %>%
  formatRound(which(sapply(hyp_te_h,is.numeric)), digits = 3)


write_csv(hyp_kegg, here("counts/hypreact.csv"))
...
DT::datatable(hyp_reactome) %>%
  formatRound(which(sapply(hyp_te_h,is.numeric)), digits = 3)

write_csv(hyp_reactome, here("counts/hyp_reactome.csv"))
