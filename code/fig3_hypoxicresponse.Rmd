...
# Acute hypoxic response histograms
...
ctrl_acute <- ggplot(data = ctrl, aes(x = LFC_rna_sictrl, y = LFC_ribo_sictrl)) + 
  ...
  geom_point(alpha=.25) +
  ylab(bquote(Log[2]~"Fold Change si-CTRL"~"RIBO: hypoxia vs normoxia")) +
  xlab(bquote(Log[2]~"Fold Change si-CTRL"~"RNA: hypoxia vs normoxia")) +
...
scatter <- ggplot(data = sig_sictrl, aes(x = LFC_te_sictrl, y = LFC_te_si3e)) + 
  ...
  geom_vline(xintercept = 0, color = "grey") +
  geom_point(alpha=.25) +
  ylab(bquote(Log[2]~"Fold Change si-eIF3e"~"TE: hypoxia vs normoxia")) +
  xlab(bquote(Log[2]~"Fold Change si-CTRL"~"TE: hypoxia vs normoxia")) +
    theme_cowplot(font_size = 16)+
  theme_prism()
...
ggsave(here("plots/acute_3e.pdf"), scatter)
...
scatter <- ggplot(data = sig_sictrl, aes(x = LFC_te_sictrl, y = LFC_te_si3d)) + 
  ...
  geom_vline(xintercept = 0, color = "grey") +
  geom_point(alpha=.25) +
  ylab(bquote(Log[2]~"Fold Change si-eIF3d"~"TE: hypoxia vs normoxia")) +
  xlab(bquote(Log[2]~"Fold Change si-CTRL"~"TE: hypoxia vs normoxia")) +
    theme_cowplot(font_size = 16)+
  theme_prism()
...
ggsave(here("plots/acute_3d.pdf"), scatter)
...
ggsave(here("plots/eIF3ebars.pdf"), eif3e_bar, width = 4, height = 6)
...
ggsave(here("plots/eIF3dbars.pdf"), eif3d_bar, width = 4, height = 6)
...
ggsave(here("plots/norm_e_d.pdf"), scatter)
...
```{r}
ggvenn(hyp_te_up, auto_scale = T, fill_color = c("#E69F00","#56B4E9"), text_size = 8)

ggvenn(hyp_te_down, auto_scale = T, fill_color = c("#E69F00","#56B4E9"), text_size=8)
...
ggsave(here("plots/hyp_e_d.pdf"), scatter)
...
ggsave(width = 4, height = 4, filename = here("plots/hypoxia_halmarks.pdf"))
ggsave(width = 4, height = 4, filename = here("plots/hypoxia_halmarks.png"))
...
write_csv(hyp_h_table, here("counts/hyp_h_table.csv"))
...
ggsave(width = 4, height = 4, filename = here("plots/hypoxia_kegg.pdf"))
ggsave(width = 4, height = 4, filename = here("plots/hypoxia_kegg.png"))
...
write_csv(hyp_kegg_table, here("counts/hyp_kegg_table.csv"))
...
ggsave(width = 4, height = 4, filename = here("plots/hypoxia_reactome.pdf"))
ggsave(width = 4, height = 4, filename = here("plots/hypoxia_reactome.png"))
...
write_csv(hyp_react_table, here("counts/hyp_react_table.csv"))
```
