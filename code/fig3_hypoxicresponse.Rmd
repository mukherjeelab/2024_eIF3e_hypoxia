...
### Pull in normoxia numbers from figure 1 
To compare normoxia to hypxia
...
where are hallmark pathway analyses?
...
## Venn diagram and pathway analysis
Separate lists by direction of change
```{r}

norm_3d_up <- res_normox_si3d %>% 
  filter(padj < 0.05 & log2FoldChange > 0) %>% pull(gene_id)

norm_3d_down <- res_normox_si3d %>% 
  filter(padj < 0.05 & log2FoldChange < 0) %>% pull(gene_id)

norm_3e_up <- res_normox_si3e %>% 
  filter(padj < 0.05 & log2FoldChange > 0) %>% pull(gene_id)

norm_3e_down <- res_normox_si3e %>% 
  filter(padj < 0.05 & log2FoldChange < 0) %>% pull(gene_id)
...
## Venn diagram of changes - normoxia - delete? is this in figure 1 or needed?
...
stopifnot(
identical(res_normox_si3d$gene_id,res_normox_si3e$gene_id))

norm_sig_te <- data.frame(
  gene_id = res_normox_si3d$gene_id,
  symbol = res_normox_si3d$symbol,
  te_3d = res_normox_si3d$log2FoldChange,
  padj_3d = res_normox_si3d$padj,
  te_3e = res_normox_si3e$log2FoldChange,
  padj_3e = res_normox_si3e$padj
) %>%
