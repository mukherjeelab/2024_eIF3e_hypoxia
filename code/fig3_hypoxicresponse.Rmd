...
# rna and ribo
all_gene_te_long_both <- all_gene_te %>% 
  melt() %>% 
  separate(variable, into = c("condition", "treatment", "rep", "type")) %>% 
  filter(treatment != "a209") %>% 
  filter(treatment != "DMSO")

all_gene_te_long_both$treatment <- relevel(factor(all_gene_te_long_both$treatment), ref = "sictrl")
all_gene_te_long_both$condition <- relevel(factor(all_gene_te_long_both$condition), ref = "normoxia")

il6st <-all_gene_te_long_both %>% 
  filter(symbol=="IL6ST")

bars_il6st <- il6st %>% 
  filter(type != "TE")

bars__summary <- bars_il6st %>%
  group_by(type, treatment, condition) %>%
  summarise(
    mean_value = mean(value),
    sd = sd(value)  # Standard deviation
  )

bars__summary$type <- factor(bars__summary$type, levels=c("rna","ribo"))

ggplot(bars__summary, aes(x = type, y = mean_value, fill = treatment)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = mean_value - sd, ymax = mean_value + sd), 
                width = 0.2, position = position_dodge(0.9)) +
...
ggsave(here("plots/il6st/il6st_rna_ribo_bars.pdf"), width = 4, height = 4.5)

bars_il6st$type <- factor(bars_il6st$type, levels=c("rna","ribo"))

#dots
ggplot(bars_il6st, aes(x = type, y = value, color = treatment)) +
  geom_point(stat = "identity", position = position_dodge()) +
  facet_wrap(~condition+treatment) +
  scale_color_manual(values = c("grey",si3d_col,si3e_col)) +
  # theme_cowplot()+
    theme(legend.position = "none")+
    labs(y= "TE", x="")


ggsave(here("plots/il6st/il6st_rna_ribo_dots.pdf"), width = 4, height = 4.5)
...
te_summary$condition <- factor(te_summary$condition, levels=c("normoxia","hypoxia"))

ggplot(te_summary, aes(x = treatment, y = mean_value, fill = treatment)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = mean_value - sd, ymax = mean_value + sd), 
                width = 0.2, position = position_dodge(0.9)) +
...
ggsave(here("plots/il6st/il6st_TE_bars.pdf"), width = 4, height = 4.5)


ggplot(il6st_te, aes(x = treatment, y = value, color = treatment)) +
  geom_point(stat = "identity", position = position_dodge()) +
  facet_wrap(~condition) +
  scale_color_manual(values = c("grey",si3d_col,si3e_col)) +
  # theme_cowplot()+
...
ggsave(here("plots/il6st/il6st_TE_dots.pdf"), width = 4, height = 4.5)
...
hyp_pathways <- list("hypoxia_hallmark_3d"=hyp_te_h_3d,
            "hypoxia_hallmark_3e"=hyp_te_h_3e,
            "hypoxia_kegg_3d"=hyp_te_kegg_3d,
            "hypoxia_kegg_3e"=hyp_te_kegg_3e)

writexl::write_xlsx(hyp_pathways, here("pathway_tables/hypoxia_3e_3d.xlsx"))
...
# Other hallmark genes in addition to HIF

```{r}
hypoxia_hallmark <- m_df_h %>% 
...
rbind(
res_normox_si3d %>%
  mutate(name = "normox_si3d"),

res_normox_si3e %>%
  mutate(name = "normox_si3e"),

res_hypox_si3d %>%
  mutate(name = "hypox_si3d"),

res_hypox_si3e %>%
  mutate(name = "hypox_si3e")
)

te_hallmark_hyp <- norm_hyp_te %>% 
  filter(symbol %in% hypoxia_hallmark$gene_symbol) %>% 
  select(symbol, log2FoldChange, padj, name)

#interested in genes down with kd

#pivot wider

wide_te_hallmark <- te_hallmark_hyp %>% 
  pivot_wider(names_from="name", values_from=c("log2FoldChange", "padj"))
```

	
IER3 up in hypoxia, up with KD in normoxia, less so in hypoxia?
PIM1m upregulated in hypxoxia up with KD



For looking into:
down with KD:
KLF6 -- insig, up in hypoxia
BTG1 -- insig, up in hypoxia
TGFB3 -- insig, down in hypoxia
UGP2 -- insit, down in hypoxia

up with KD:
FOS -- insig, up in hypxoia
PKP1 -- sig, down in hypxoia -- this could look good

add in a209 changes and make sure they are consistent
```{r}


hyp_209 <- read_csv(here("counts/fig6/hypoxia_dmso_a209.csv")) %>% 
  dplyr::select(symbol, log2FoldChange, padj) %>% 
  rename("log2FoldChange_hyp_209" = "log2FoldChange", "padj_hyp_209"="padj")

wide_te_hallmark_all <- left_join(wide_te_hallmark, hyp_209)

write_csv(wide_te_hallmark_all, "hallmark_gene_te_changes.csv")

```


Ask: what happened with these genes in acute hypoxia?
```{r}

acute_all_changes <- all_changes %>% 
  dplyr::select(symbol, LFC_te_sictrl, padj_te_sictrl)


all_changes_hypoxic_hallmark <- all_changes %>% 
    dplyr::select(symbol, LFC_te_sictrl, padj_te_sictrl, LFC_te_si3e, padj_te_3e) %>% 
  filter(symbol %in% hypoxia_hallmark$gene_symbol)
...
Number of genes changed with 3d and 3e KD in the hypoxia pathway

```{r}

normoxia_hallmark_hypoxia_te <-
rbind(
res_normox_si3d %>%
  mutate(name = "normox_si3d"),
...
normoxia_hallmark_hypoxia_te <- normoxia_hallmark_hypoxia_te %>% 
  filter(symbol %in% hypoxia_hallmark$gene_symbol) %>% 
  select(symbol, log2FoldChange, padj, name)
...
```{r}
number_changed_3d <- normoxia_hallmark_hypoxia_te %>% 
  filter(name=="normox_si3d") %>% 
  filter(padj < 0.05) %>% 
  dplyr::select(symbol, log2FoldChange)

number_changed_3d$symbol


```

```{r}
number_changed_3e <- normoxia_hallmark_hypoxia_te %>% 
  filter(name=="normox_si3e") %>% 
  filter(padj < 0.05) %>% 
  dplyr::select(symbol, log2FoldChange)

number_changed_3e$symbol
...
```{r}
acute_test <- res_acute_sictrl %>% 
  filter(padj < 0.05)

biggest_up <- acute_test %>% 
  filter(log2FoldChange > 1)

biggest_down <- acute_test %>% 
  filter(log2FoldChange < -1)
```
