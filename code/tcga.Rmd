...
# TCGA data - read in z scores and matching clinical data
```{r}
zscores <- read.table(here("cbioportal_data/tcga_data_mrna_seq_v2_rsem_zscores_ref_diploid_samples.txt"), sep="\t", header=T)
...
```{r}
clin <- read.table(here("cbioportal_data/tcga_data_clinical_patient.txt"), sep="\t", header=TRUE, comment.char="", quote="")
...
patient_scores$patient <- sub("-01$", "", patient_scores$patient)



all_data <- left_join(clin_small, patient_scores, by=c("PATIENT_ID"="patient"))
...
# all_data_clean <- na.omit(all_data$OS_STATUS)
# all_data$OS_STATUS <- gsub(",", ".", all_data$OS_STATUS)
# 
# all_data$OS_YEARS <- as.numeric(all_data$OS_YEARS)
# 
# str(all_data$quantile)
# 
# 
# all_data_arranged
...
alive_data <- all_data
#   mutate(alive = ifelse(OS_STATUS=="Living", TRUE, FALSE))
...
neg_ctrl_fake_patient_scores$patient <- sub("-01$", "", as.character(neg_ctrl_fake_patient_scores$patient))
...
all_data <- left_join(clin_small, neg_ctrl_fake_patient_scores, by=c("PATIENT_ID"="patient"))
...
```{r}
clin_strat <- all_data[c(which(all_data$neg_ctrl_low_signature), which(all_data$neg_ctrl_high_signature)),]
fit <- survfit(Surv(OS_YEARS, OS_STATUS) ~ neg_ctrl_low_signature, data = clin_strat)


ggsurvplot(fit, data = all_data, size = 1, palette = c("#E7B800", "#2E9FDF"),legend.labs = c("NEG CTRL eIF3e KD signature high", "NEG CTRL eIF3e KD signature low"),  break.time.by = 1, ylab="Overall Survival", xlab = "Time in years", xlim = c(0,10),conf.int = TRUE, pval = TRUE, risk.table = TRUE,  risk.table.col = "strata", risk.table.height = 0.25, ggtheme = theme_bw())
